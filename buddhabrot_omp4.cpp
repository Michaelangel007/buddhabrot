...
// Globals
    ...
    // Input parameters
    float     gnWorldMinX        = -2.102613; // WorldW = MaxX-MinX = 3.303226
    float     gnWorldMaxX        =  1.200613;
    float     gnWorldMinY        = -1.237710; // WorldH = MaxY-MinY = 2.47742 
...
{
    float   r = 0.f, i = 0.f; // Zn   current Complex< real, imaginary >
    float   s      , j      ; // Zn+1 next    Complex< real, imaginary >
    ...
    for( int depth = 0; depth < maxdepth; depth++ )
    {
        ...
        if ((r*r + i*i) > 4.0f ) // escapes to infinity, don't render
            return;
...
// END OMP
        ...
        /* */ float     r = 0.f, i = 0.f, s, j;
            ...
            for (int depth = 0; depth < gnMaxDepth; depth++)
            {
                ...
                if ((r*r + i*i) > 4.0f) // escapes to infinity so trace path
                {
...
// END OMP
        {
            ...
            {
                const float percent = (100.0f * iCel) / nCel;
...
{
    printf(
"Buddhabrot (OMP) v1.12 by Michael Pohoreski\n"
...
int main( int nArg, char * aArg[] )
{
    ...
    if( gbSaveRawGreyscale )
    {
        ...
        sprintf( filenameRAW, "raw_omp4_buddhabrot_%dx%d_%d_%dx.u16.data", gnWidth, gnHeight, gnMaxDepth, gnScale );
...
#if DEBUG
    sprintf( filenameBMP, "omp4_buddhabrot_%dx%d_depth_%d_colorscaling_%d_scale_%dx.bmp", gnWidth, gnHeight, gnMaxDepth, (int)gbAutoBrightness, gnScale );
#else
    sprintf( filenameBMP, "omp4_buddhabrot_%dx%d_%d.bmp", gnWidth, gnHeight, gnMaxDepth );
#endif
